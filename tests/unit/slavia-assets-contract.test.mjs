...
test("Slavia manifest integrity matches physical production asset files", () => {
  ...
  assert.equal(slaviaEntries.some(entry => /\.svg$|\.gltf$/i.test(entry.url)), false, "runtime Slavia pack contains placeholder SVG/glTF");
});
...
test("Slavia data and production scene references resolve through the manifest", () => {
  ...
  for (const id of [
    ...
    assert.ok(ids.has(id), `manifest missing data asset ${id}`);
  }
