...
export class BesedniceScene {
  ...
  async createVisualWorld() {
    ...
    const [environmentTexture, foregroundTexture, playerTexture, milanAtlasTexture, karelAtlasTexture] = await Promise.all([
    ...
    const milanAnimator = createAnimatedNPC(
      ...
      { width: 78, height: 104, z: 12, anchorX: 0.5, anchorY: 0.08 }
    );
  ...
  updateAnimations(dt) {
    ...
    if (this.milanAnimator) this.milanAnimator.update(deltaMs);
    if (this.karelAnimator) this.karelAnimator.update(deltaMs);
  ...
  showGuideDialog() {
    ...
    this.modal = "dialog";
    if (this.milanAnimator) playDialogueAnimation(this.milanAnimator, "start");
  ...
  snapshot() {
    ...
    return {
      ...
      runtime: {
        ...
        milanAnimation: this.milanAnimator?.getState().animation ?? null,
  ...
  onDigHit() {
    if (!this.dustEmitter || !Number.isInteger(this.hedgehogEntity)) return;
    const transform = this.app.world.get(this.hedgehogEntity, "transform");
    if (!transform) return;
    this.dustEmitter.emitBurst(transform.x, transform.y, 4, 10, {
      speed: 2.2,
      spread: 0.6,
      lifetime: 0.4,
      size: 2.2
  ...
  onDigClean() {
    if (!this.sparkleEmitter || !Number.isInteger(this.hedgehogEntity)) return;
    const transform = this.app.world.get(this.hedgehogEntity, "transform");
    if (!transform) return;
