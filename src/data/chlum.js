...
const searchPositions = getLevelTarget("chlum", "chlum-search-site")?.positions;
if (!level || !farmerPosition || searchPositions?.length !== 3 || !getDialogueDefinition("chlum-permission")) throw new Error("Chlum canonical data is incomplete.");
...
const entities = [
  ...
  ...searchPositions.map((position, index) => ({
    id: index === 0 ? "chlum-search-site" : `chlum-search-site-${index + 1}`,
    components: {
      transform: { ...position, rotation: 0, scale: 1 },
      model: { assetId: "model-chlum-field-marker", layer: "props" },
      searchSpot: { findingId: `chlum-finding-${index + 1}`, searched: false, detectionRange: 240 }
    }
  })),
